...
try {
    ...
    foreach ($returnIds as $loanId) {
        ...
        audit_log($db, 'return', 'loan', $loanId, 'Ausleihe zurückgegeben', [
            'bauteil_id' => (int)$loan['Bauteil_ID'],
        ]);
        audit_log($db, 'stock_increase', 'inventory', (int)$loan['Bauteil_ID'], 'Bestand durch Rückgabe erhöht', [
            'loan_id' => $loanId,
        ]);
        $processed++;
    }
